...
__Code Project 2__ <br>
__due:__ March 3 at 11:59pm
...
<div class="updated" markdown="1">
The `RecordList` class is used to store all the `Record` objects parsed from the input file. Unlike Project 2, this class must now inherit from your custom `LinkedList<E>` class (see below for the specification).
...
#### <code class="code_title">LinkedList</code> class

<div class="updated" markdown="1">

For this project, you are not allowed to use Java's built-in collection classes (like `java.util.LinkedList`). Instead, you must implement your own **generic, singly linked list**. This class will serve as the foundation for storing your log records (but as a generic list, it can also be used in many other contexts).

The class must be generic, it should be defined as `public class LinkedList<E>`.

* **Inner Node Class:** You should implement a private inner class `Node` to represent the elements in the list.
* **Iterator:** The class must implement the `Iterable<E>` interface and provide a functional iterator as an inner class.
* **Constructor:** The class must provide a default constructor that creates an empty list.

In addition, the class should provide the following methods: 

* `public void add(E element)`: Adds a new element to the end of the list. Throws a `IllegalArgumentException` if `element` is `null`. 
* `public E get(int index)`: Returns the element at the specified position. Throws `IndexOutOfBoundsException` if the index is out of range.
* `public boolean contains(Object o)`: Returns `true` if this list contains the specified element, and `false` otherwise.
...
* `public E remove(int index)`: Removes and returns the element at the specified position. Throws `IndexOutOfBoundsException` if the index is out of range.
* `public boolean equals(Object o)`: Compares the specified object with this list for equality.  Two lists are considered equal if and only if:
	* The other object is also an instance of `LinkedList`.
	* Both lists are of the same size.
	* Both lists contain the same elements in the **exact same order**. This should be checked by calling the `.equals()` method on the individual elements stored within the nodes.
* `public String toString()`: Returns a string representation of the list.  
	The returned string must start with `[` and end with `]`. The elements of the list should be separated by a comma and a single space. For example: `[element1, element2, element3]`. An empty list should return `[]`. (This matches the behavior of `java.util.ArrayList.toString()`).

To simplify your implementation (and to illustrate how a class should be structured and documented) the end of this project specification contains a stub file for the above `LinkedList` class. Use it to guide you through **your own implementation**. 
...
- You may **not** use `java.util.ArrayList`, `java.util.LinkedList`, or any other predefined collection classes from the Java Collections Framework. All data must be stored in your custom `RecordList` class which is a specialized `LinkedList` that is implemented by you. 

- Ensure `RecordList` correctly leverages the methods you wrote in `LinkedList`. Do not "re-implement" list logic inside `RecordList` if it already exists in the parent class.
...
<table class="checklist-table">
  ...
  <tbody>
    <tr>
      ...
      <td>Does every file start with <code>package project2;</code>? Remember that package names are case sensitive. </td>
...
#### <code class="code_title">LinkedList</code> stub 
...
// 
// THIS CLASS SHOULD BE IMPLEMENTED BY YOU.  
// YOU SHOULD NOT ASK GENERATIVE AI TO IMPLEMENT IT FOR YOU.
// IF YOU NEED HELP WITH THE IMPLEMENTATION VISIT OUR OFFICE
// HOURS OR TUTORING HOURS. 
// YOU MAY NEED TO IMPLEMENT PARTS OF THIS CLASS IN SITUATION
// IN WHICH YOU DO NOT HAVE ACCESS TO ANY GENERATION TOOLS. 
// 
// YOU ARE RESPONSIBLE FOR BEING ABLE TO EXPLAIN EVERY LINE 
// OF CODE THAT YOU SUBMIT. 
//
...
public class LinkedList<E> implements Iterable<E> {
    ...
    // TODO: Add instance variables (e.g., head, size)
    ...
    /**
     * Returns a string representation of the list in the format [e1, e2, ...].
    ...
    private class LinkedListIterator implements Iterator<E> {
        // TODO: Add instance variables for tracking position
